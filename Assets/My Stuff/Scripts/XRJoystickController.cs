...
public class XRJoystickController : UnityEngine.XR.Interaction.Toolkit.Interactables.XRGrabInteractable
{
    ...
    void Update()
    {
        ...
        else if (handle != null && !isSelected)
        {
            // Return handle to center when not grabbed
            handle.localRotation = Quaternion.Slerp(
                handle.localRotation, 
                Quaternion.Euler(initialHandleRotation), 
    ...
    private void UpdateHandleVisuals(float dx, float dz)
    {
        if (handle != null)
        {
            // Default to initial rotation
            Quaternion targetRotation = Quaternion.Euler(initialHandleRotation);
            bool shouldTilt = false;
            
            // X-axis tilt (left/right)
            if (enableXMovement && Mathf.Abs(dx) > deadZone)
            {
                float zAngle = dx > 0 ? -maxAngle : maxAngle;
                targetRotation = Quaternion.Euler(initialHandleRotation.x, initialHandleRotation.y, zAngle);
                shouldTilt = true;
            }
            // Z-axis tilt (forward/back) - only apply if X isn't active
            else if (enableZMovement && Mathf.Abs(dz) > deadZone)
            {
                float xAngle = dz > 0 ? maxAngle : -maxAngle;
                targetRotation = Quaternion.Euler(xAngle, initialHandleRotation.y, initialHandleRotation.z);
                shouldTilt = true;
            }
